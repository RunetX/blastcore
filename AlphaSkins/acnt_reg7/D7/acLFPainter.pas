...
// Tested and compatible with DevEx v6.45

{.$DEFINE VER645} // Disable this key for support DevEx v6.41-6.44
...
uses
  ...
  sSkinManager, sStyleSimply, sMaskData, cxLookAndFeelPainters, cxGraphics, cxClasses, ImgList;
...
//    class procedure CalculateArrowPoints(R: TRect; var P: TcxArrowPoints; AArrowDirection: TcxArrowDirection; AProportional: Boolean; AArrowSize: Integer = 0);
    ...
    class procedure DrawButton(ACanvas: TcxCanvas; R: TRect; const ACaption: string;
      ...
      ATextColor: TColor = clDefault; AWordWrap: Boolean = False{$IFNDEF VER640}; AIsToolButton: Boolean = False{$ENDIF}); override;
...
function Skinned : boolean; begin if DefaultManager <> nil then Result := DefaultManager.SkinData.Active else Result := False end;

{function PaintSection(const Bmp : TBitmap; Section : string; const SecondSection : string; const State : integer; const ParentOffset : TPoint; ParentDC : hdc = 0) : integer;
var
  CI : TCacheInfo;
begin
  Result := DefaultManager.GetSkinIndex(Section);
  if not DefaultManager.IsValidSkinIndex(Result) then begin
    Section := SecondSection;
    Result := DefaultManager.GetSkinIndex(Section);
  end;
  if DefaultManager.IsValidSkinIndex(Result) then begin
...
begin // -
  if Skinned then begin
    if DefaultManager.ConstData.CheckBoxChecked < 0 then Result := inherited CheckButtonSize else begin
      Result.cx := WidthOfImage(DefaultManager.ma[DefaultManager.ConstData.CheckBoxChecked]);
      Result.cy := HeightOfImage(DefaultManager.ma[DefaultManager.ConstData.CheckBoxChecked]);
    end;
  end
...
begin
  if Skinned then Result := DefaultManager.GetGlobalColor else Result := inherited DefaultVGridCategoryTextColor;
end;

class function TcxACLookAndFeelPainter.DefaultVGridLineColor: TColor;
begin
  if Skinned then Result := DefaultManager.GetGlobalColor else Result := inherited DefaultVGridLineColor;
end;

class function TcxACLookAndFeelPainter.DefaultVGridBandLineColor: TColor;
begin
  if Skinned then Result := DefaultManager.GetGlobalColor else Result := inherited DefaultVGridBandLineColor;
end;
...
begin // +
  if Skinned then Result := {MixColors(}DefaultManager.GetActiveEditColor{, DefaultManager.GetGlobalColor, 1)} else Result := inherited DefaultContentEvenColor;
end;
...
begin
  if Skinned then Result := DefaultManager.GetGlobalColor else Result := DefaultDateNavigatorHeaderColor;
end;
...
begin // +
  ...
  else Result := DefaultEditorBackgroundColor(AIsDisabled);
end;
...
begin
  if Skinned then begin
    case ACheckState of
      cbsUnchecked : i := DefaultManager.ConstData.CheckBoxUnChecked;
      cbsChecked : i := DefaultManager.ConstData.CheckBoxChecked;
      cbsGrayed : i := DefaultManager.ConstData.CheckBoxGrayed; 
    end;
...
class procedure TcxACLookAndFeelPainter.DrawEditorButton(ACanvas: TcxCanvas; const ARect: TRect; AButtonKind: TcxEditBtnKind; AState: TcxButtonState);
var
  i, glIndex : integer;
  TmpBmp : TBitmap;
  State : integer;
...
begin
  if Skinned then begin
    ...
    if DefaultManager.IsValidImgIndex(i) then begin
      ...
      CI := MakeCacheInfo(TmpBmp);
      DrawSkinGlyph(TmpBmp, Point((WidthOf(R) - WidthOfImage(DefaultManager.ma[i])) div 2, (HeightOf(R) - HeightOfImage(DefaultManager.ma[i])) div 2), State, 1, DefaultManager.ma[i], CI);
  ...
  if Skinned then begin
    if AState in [cxbsDefault, cxbsNormal, cxbsHot, cxbsPressed] then begin
      ...
      if (w > 0) and (h > 0) then begin
        ...
        aBmp := CreateBmp24(WidthOf(R, True), HeightOf(R, True));
...
begin
  if Skinned then begin
    Bmp := CreateBmp24(WidthOf(ABorderRect), HeightOf(ABorderRect));
...
begin
  if Skinned then begin
    ...
    if DefaultManager.IsValidSkinIndex(i) then begin
      TmpBmp := CreateBmp24(WidthOf(ARect), HeightOf(ARect));
      CI.FillColor := DefaultManager.GetGlobalColor;
      CI.Ready := False;
...
begin
  if Skinned then begin
    ...
    if DefaultManager.IsValidSkinIndex(i) then begin
      TmpBmp := CreateBmp24(WidthOf(ARect), HeightOf(ARect));
      BGBmp := CreateBmp24(WidthOf(ARect), HeightOf(ARect));
...
begin
  if Skinned then begin
    ...
    if AIsNextButton
      then PaintSection(TmpBmp, s_TabLeft, s_Button, GetState[AState], DefaultManager, ARect.TopLeft, DefaultManager.GetGlobalColor, ACanvas.Handle)
      else PaintSection(TmpBmp, s_TabRight, s_Button, GetState[AState], DefaultManager, ARect.TopLeft, DefaultManager.GetGlobalColor, ACanvas.Handle);
