...
uses
  ...
  sPanel, sSpeedButton, sCommonData, ImgList;
...
type
  ...
  TacScrollPanelBand = class(TCustomPanel)
  private
    FData: Pointer;
...
begin
  if Shift = [ssLeft] then begin
    Sm := FY - Y;
    T := Top;
    if Sm <> 0 then with Parent as TacPanelScrollBar do begin
      OldPos := Pos;
...
begin
  with (Owner as TacPanelScrollBar) do Msg.Result := Integer(FDesignInteractive and (FPage <> FMax - FMin + 1));
end;
...
begin
  ...
  if (csDesigning in AOwner.ComponentState) then ControlStyle := ControlStyle - [csAcceptsControls];
  Scroll.Caption := '';
...
begin
  if (Parent <> nil) and TacScrollPanelBand(Parent).Expanded then Result := 1 else Result := inherited CurrentState
end;

procedure TacBandBtn.FontChanged;
begin
  if Parent <> nil then with Parent as TacScrollPanelBand do begin
...
begin
  Result := FOrder;
  if (Parent = nil) or not (Parent is TacScrollPanelBands) then Exit;
...
begin
  if (Parent <> nil) and (Parent is TacScrollPanelBands) and ((Parent as TacScrollPanelBands).Parent <> nil)
    then Result := Parent.Parent as TacScrollPanel
    else Result := nil;
end;
...
var
  W, H: Integer;
begin
  if (Parent <> nil) and not (Parent is TacScrollPanelBands) then Exit;
  if FExpanded then H := FExpandedHeight else H := CollapsedHeight;
  if not Visible then H := 0; 
...
var
  R: TRect;
begin
  if not SkinData.Skinned then if Canvas.Handle <> 0 then begin
...
{  if (AControl <> nil) and not (AControl is TacScrollPanelBand) then begin
    raise Exception.Create('TacScrollPanelBand control only may be placed here');
  end;}
  if Parent is TacScrollPanel then ScrollMax := Parent as TacScrollPanel else begin
    Exit
  end;
  if (AControl <> nil) and (AControl is TacScrollPanelBand) and (AControl as TacScrollPanelBand).FExpanded and ScrollMax.FOneExpanded then
    for I := 0 to ControlCount - 1 do
      if Controls[I] <> AControl then (Controls[I] as TacScrollPanelBand).Expanded := False;
  SPos := ScrollMax.FScrollPos;
  if ControlCount > 0 then begin
    for I := 0 to ControlCount - 1 do begin
      if I > 0 then T := Controls[I - 1].BoundsRect.Bottom else T := -ScrollMax.FScrollPos;
      if (Controls[I] is TacScrollPanelBand) then (Controls[I] as TacScrollPanelBand).UpdateSize(T);
    end;
    AdjustBottom;
    if (AControl is TacScrollPanelBand) then AdjustBand;
...
begin
  if (Control <> nil) and ContainsControl(Control) and (Parent <> nil) then (Parent as TacScrollPanel).ScrollInView(Control);
end;
...
var
  I, iB, iC: Integer;
begin
  for I := Low(AControlClasses) to High(AControlClasses) do for iB := 0 to BandCount - 1 do for iC := 0 to Bands[iB].ControlCount - 1 do if Bands[iB].Controls[iC] is AControlClasses[I] then begin
...
begin
  if (Button = mbLeft) and (BandCount > 0) then FY := (Sender as TControl).ClientToScreen(Point(0, Y)).Y;
end;
...
begin
  if (ssLeft in Shift) and (BandCount > 0) then begin
    ...
    if Sm > 0 then begin // Down
      if not (Bands[0].Top < 0) then Sm := 0 else if Bands[0].Top + Sm > 0 then Sm := -Bands[0].Top;
    end;
    if Sm <> 0 then begin
      FPnlEdit.ScrollControls(Sm);
      FScrollBar.Pos := -Bands[0].Top;
...
begin
  Band := nil;
  for I := 0 to FPnlEdit.ControlCount - 1 do if (FPnlEdit.Controls[I] as TacScrollPanelBand).ContainsControl(AControl) then begin
...
procedure TacScrollPanelBand.PrepareCache;
var
  CI : TCacheInfo;
  h : integer;
begin
  SkinData.InitCacheBmp; // Initializing of acache Bmp
  CI := GetParentCache(SkinData); // Receive information about parent BG image

  if ButtonVisible then begin
    h := FButton.Height + 1;
    // Draw BG under button
    if CI.Ready // If we have image as Bitmap
      then BitBlt(SkinData.FCacheBMP.Canvas.Handle, 0, 0, Width, h, CI.Bmp.Canvas.Handle, Left + CI.X, Top + CI.Y, SrcCopy)
      // If we have color for filling only
      else FillDC(SkinData.FCacheBMP.Canvas.Handle, Rect(0, 0, Width, h), CI.FillColor);
    if Expanded then PaintItem(SkinData, CI, False, 0, Rect(0, FButton.Height, Width, Height), Point(Left, Top + h), SkinData.FCacheBMP, False);
  end
  else PaintItem(SkinData, CI, True, 0, Rect(0, 0, Width, Height), Point(Left, Top), SkinData.FCacheBMP, True);
//  SkinData.FCacheBMP.SaveToFile('c:\FCacheBMP.bmp');
...
begin
  ...
  if not SkinData.Updating then begin // We can draw if control is not in updating process
    ...
    if b then PrepareCache; // Prepare image of control
    CopyWinControlCache(Self, SkinData, Rect(0, 0, 0, 0), Rect(0, 0, Width, Height), NewDC, True); // Out image to control device context
    sVCLUtils.PaintControls(NewDC, Self, True, Point(0, 0)); // Paint graphic controls 
    if SendUpdated then SetParentUpdated(Self); // Send message to children who must be updated (background is ready now)
  end;
end;
...
//        RedrawWindow(Handle, nil, 0, RDW_FRAME or RDW_INVALIDATE);
  ...
  if not ControlIsReady(Self) or not SkinData.Skinned then inherited else begin
    if Message.Msg = SM_ALPHACMD then case Message.WParamHi of
      ...
      AC_PREPARING : begin // If bkground is not ready then Result is 1
      ...
      case Message.Msg of
        ...
        WM_MOVE : SkinData.BGChanged := True; // Full update of client after each panel scrolling
      ...
      if CommonWndProc(Message, SkinData) then Exit; // Common handler for all controls in the package / sCommonData.pas
      inherited;
      case Message.Msg of
        ...
        WM_SETFONT : if Caption <> '' then SkinData.Invalidate;
      end;
    end;
  end;
end;
...
begin
  inherited;
{  if SkinData.Skinned then case Message.Msg of
    WM_SIZE : begin
      SkinData.BGChanged := True;
      RedrawWindow(Handle, nil, 0, RDW_ERASE or RDW_ALLCHILDREN or RDW_INVALIDATE or RDW_UPDATENOW);
      InvalidateRect(Handle, nil, True);
    end;
  end;}
end;
